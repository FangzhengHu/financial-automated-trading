...
# The Know Sure Thing (KST) is a momentum oscillator based on the smoothed rate-of-change for four different time frames.
...
getOrders = function(store, newRowList, currentPos, params) {
  ...
  if(store$iter >= params$iniDay) {
    for (i in params$series) {
      ...
      open = store$op[[i]][dataIndex]
      
      Kst = last(KST(open))
      
      # determine the long/short position by using oscillator rules
      if(Kst$kst>Kst$signal && Kst$kst>0)
        pos[i] = 1 * store$posSize[i]
      else if(Kst$kst<Kst$signal && Kst$kst<0)
        pos[i] = -1 * store$posSize[i]
      else
        pos[i] = currentPos[i]
    }
