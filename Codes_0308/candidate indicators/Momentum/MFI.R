...
getOrders = function(store, newRowList, currentPos, params) {
  ...
  if (store$iter >= params$iniDay) {
    for (i in params$series) {
      ...
      Mfi = MFI(hlc,vol,n=params$lookback)
      
      Mfi_signal = SMA(Mfi,n=params$lookback_sig)
      
      if (last(Mfi)>last(Mfi_signal) && last(Mfi)>50+params$threshold)
        pos[i] = 1 * store$posSize[i]
      else if (last(Mfi)<last(Mfi_signal) && last(Mfi)<50-params$threshold)
        pos[i] = -1 * store$posSize[i]
      else
        pos[i] = currentPos[i]
    }
