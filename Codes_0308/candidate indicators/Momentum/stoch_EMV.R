...
# this strategy combines two indicators Stochastic and EMV
# Stochastic oscillator can detect prices in overbought or oversold levels
# Ease of Movement is a volume-based oscillator
# prices are advancing with relative ease when the EMV is in positive territory
# prices are declining with relative ease when the EMV is in negative territory
...
getOrders = function(store, newRowList, currentPos, params) {
  ...
  if (store$iter >= params$iniDay) {
    for (i in params$series) {
      ...
      hl = cbind(store$hi[[i]][dataIndex], store$lo[[i]][dataIndex])
      
      vol = store$vo[[i]][dataIndex]
      
      Stoch = last(stoch(hlc, nFastK=params$fkLookback, nFastD=params$fdLookback, nSlowD=params$sdLookback))
      
      Emv = last(EMV(hl, vol, n=params$lookback_emv))
      
      # determine the long/short position by using oscillator rules
      if(Stoch$fastD>Stoch$slowD && Stoch$fastD>0.5+params$threshold && Emv$maEMV>0)
        pos[i] = 1 * store$posSize[i]
      else if(Stoch$slowD>Stoch$fastD && Stoch$fastD<0.5-params$threshold && Emv$maEMV<0)
        pos[i] = -1 * store$posSize[i]
      else
        pos[i] = currentPos[i]
    }
