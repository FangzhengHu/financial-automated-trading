...
getOrders = function(store, newRowList, currentPos, params) {
  ...
  if (store$iter >= params$iniDay) {
    for (i in params$series) {
      ...
      Wpr = WPR(hlc, n=params$lookback)
      
      Wpr_signal = SMA(Wpr,n=params$lookback_sig)

      if(last(Wpr)<last(Wpr_signal) && last(Wpr)<0.5-params$threshold) {
        pos[i] = 1 * store$posSize[i]
      }
      else if(last(Wpr)>last(Wpr_signal) && last(Wpr)>0.5+params$threshold) {
        pos[i] = -1 * store$posSize[i]
      }
      else
        pos[i] = currentPos[i]
